...
observeEvent(input$show_FDC, {
    if(input$show_FDC){
      leafletProxy("map1", data=shape_join()) %>%
        addPolygons(data = shape_join(),
                  fillColor = ~community_type,
...
observeEvent(input$show_income, {
...
fcd_acs <- reactive({inner_join(newdf(),acs(), by = c("STATEFP" = "state", "COUNTYFP" = "county"))})

renderPlotly({
  ggplotly(
    ggplot(data = fcd_acs(), aes_string(x = input$selected_varX, y = input$selected_varY, size = input$selected_varZ)) +
      geom_point(alpha = 0.5) + scale_size(range = c(.1, 5))+
      labs(x = " ",
           y = " ") + facet_wrap(~fcd_acs()$"community_type") +theme(axis.text.x = element_text(angle = 90))
  )
  })
```
...
renderPlotly({
  ggplotly(
    ggplot() +
      geom_point(data = fcd_acs(), aes_string(x = fcd_acs()$"community_type", y = input$selected_varA))
      +
      labs(x = " ",
           y = " ")) 
  })
...
renderPlotly({
  ggplotly(
    ggplot(data = fcd_acs(), aes_string(x =fcd_acs()$"community_type", y = input$selected_varB)) +
